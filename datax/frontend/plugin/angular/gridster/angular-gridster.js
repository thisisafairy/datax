				...
				if (this.sparse === false) { // check all cells
					for (var h = 0; h < sizeY; ++h) {
						for (var w = 0; w < sizeX; ++w) {
							...
							if (item && (!excludeItems || excludeItems.indexOf(item) === -1) && items.indexOf(item) === -1&&item.freelayout!==1) {
								items.push(item);
							}
						}
					}
					...
					for (var i = 0; i < this.allItems.length; ++i) {
						...
						if (item && (!excludeItems || excludeItems.indexOf(item) === -1) && items.indexOf(item) === -1 && this.intersect(item, column, right, row, bottom)&&item.freelayout!==1) {
							items.push(item);
						}
					}
				}
			...
			this.getBoundingBox = function(items) {
				...
				if (items.length === 1) {
					return {
						...
						sizeY: items[0].sizeY,
						sizeX: items[0].sizeX,
						freelayout: items[0].freelayout,
						left: items[0].left,
						top: items[0].top,
						screenWidth: items[0].screenWidth
					};
				}
		...
		function($timeout, $window, $rootScope, gridsterDebounce) {
			return {
				...
				compile: function($tplElem) {
					...
					$tplElem.prepend('<div ng-if="gridster.movingItem&&gridster.movingItem.freelayout!==1" gridster-preview></div>');
					...
					return function(scope, $elem, attrs, gridster) {
						...
						var resize = function() {
							...
							//同比改变浮动item坐标,setposition处统一处理
							if(gridster.allItems&&gridster.allItems.length>0)
							{
								for(var it=0;it<gridster.allItems.length;it++)
								{
									var ite = gridster.allItems[it];
									if(ite.freelayout!==1)continue;
									if(ite.screenWidth!=gridster.curWidth)
									{
										//ite.top = ite.top*width/prevWidth;
										ite.left = ite.left*width/prevWidth;
										ite.screenWidth = gridster.curWidth;
									}
								}
							}
							prevWidth = width;
	...
	.controller('GridsterItemCtrl', function() {
		...
		this.setElementPosition = function() {
			if (this.gridster.isMobile) {
				...
				if(this.freelayout===1)
				{
					if(this.gridster.curWidth!==this.screenWidth)//同比例改变,兼容其它客户端打开时,重新计算
					{
						this.left = this.left*this.gridster.curWidth/this.screenWidth;//同比例缩放
						this.screenWidth = this.gridster.curWidth;//记录最新的屏幕宽度
					}
					this.$element.css({
						margin: 0,
						top: this.top + 'px',
		...
		function($document, $window, GridsterTouch) {
			function GridsterDraggable($el, scope, gridster, item, itemOptions) {
				...
				function dragStop(event) {
					...
					if(item.freelayout===1)
					{
						item.left = elmX;
						item.top = elmY;
						item.screenWidth = gridster.curWidth;//记录当前屏幕宽度
		...
		function($parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
			return {
				...
				link: function(scope, $el, attrs, controllers) {
					...
					if (optionsKey) {
						...
						if (!options && $optionsGetter.assign) {
							options = {
								row: item.row,
								col: item.col,
								freelayout: item.freelayout||0,
								left: item.left||0,
								top: item.top||0,
								screenWidth: item.screenWidth||0,
								sizeX: item.sizeX,
								sizeY: item.sizeY,
								minSizeX: 0,
								minSizeY: 0,
					...
					var aspects = ['minSizeX', 'maxSizeX', 'minSizeY', 'maxSizeY', 'sizeX', 'sizeY', 'row', 'col','freelayout','left','top','screenWidth'],
						$getters = {};
					...
					scope.$watch(function() {
						return item.freelayout;
					}, function(){
						item.freelayout===1?$el.addClass('freelayout'):$el.removeClass('freelayout');
					});
					...
					function positionChanged() {
						...
						if ($getters.left && $getters.left.assign) {
							$getters.left.assign(scope, item.left);
						}
						if ($getters.top && $getters.top.assign) {
							$getters.top.assign(scope, item.top);
						}
						if ($getters.screenWidth && $getters.screenWidth.assign) {
							$getters.screenWidth.assign(scope, item.screenWidth);
						}
						if ($getters.freelayout && $getters.freelayout.assign) {
							$getters.freelayout.assign(scope, item.freelayout);
						}
					}
					scope.$watch(function() {
						return item.row + ',' + item.col+','+item.left+','+item.top;
					}, positionChanged);
