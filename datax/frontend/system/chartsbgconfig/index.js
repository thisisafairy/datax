...
app.controller('chartsbgconfigController', function ($scope, $http, $uibModal) {
    ...
    // 保存编辑完刷新页面
    $scope.addconfig = function(id){
        let url = "/dashboard/newaddscenebackgroud?id="+id;
        window.open(url);
    };
...
//弹出保存对话框，填写名称等信息保存到数据库
app.controller('saveConfigController', function ($scope,$timeout, $http, $uibModalInstance, id, sceneConfigObj) {
    $scope.errorflag = false;
    $scope.checking = false;
    $scope.confnamemsg = id=='0'?"*名称必填":"";

    $scope.sceneConfigObj = angular.copy(sceneConfigObj);
    ...
    $scope.cancel = function () {
        $uibModalInstance.dismiss('cancel');
        $(".modal-content").off("draggable");//当关闭的时候移除拖拽以免其他窗口可以拖动
    };
    // $timeout(function () {
