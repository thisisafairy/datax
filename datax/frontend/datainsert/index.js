 ...
 */
  ...
  */
    ...
    $scope.getTable = function (fn) {
        ...
        $scope.draged = [];
        $scope.dataJson = [];
        $scope.proposeColumns = [];//当拖入表时，需要用户配置哪些字段可用、可groupby、可sum，拖入表格的形式需要从这里取字段数据
        // $scope.datagrid.columns = [];
        $scope.datagrid.lists = [];
        $scope.fulltable = [];
        $scope.searched = "";
...
//下一页
  ...
  */
    ...
    /**
     * 每次编辑，需要将编辑后的字段覆盖更新全部（当前表）到$scope.proposeColumns
     * 需要重新自动关联当前表的所有子表（有可能你当前写的sql把旧的关联字段写没了）
     * @param item
     */
        ...
        if ($scope.selectedSource && $scope.selectedSource != '') {
            ...
            modalInstance.result.then(function (rs) {
                if (rs.status == 'success') {
                    ...
                    //以下使用的userDefinedSql在sqltool.py中也有对应的处理，如需修改需要兼顾,draged由于是json字符串所以可以添加其他字段
                    var currTbNm = item.table;
                    for (var i = 0; i < $scope.draged.length; i++) {
                    ...
                    $scope.proposeColumns = $scope.proposeColumns.filter(function (itemObj) {//先删除以前的字段，再新增
                        return itemObj.table != currTbNm;
                    });
                    ...
                    if ($scope.draged.length > 1) {//如果自定义sql是关联的主表，当它修改时，它的所有子表都需要重新关联
                        //一下代码是为了找到原始关联的target主表，对当前自定义sql和原始表进行重新关联
                        var idx, oldDJObj = '';
                        $scope.dataJson.map(function (itemObj, idx) {//遍历dataJson里的所有项，找到以当前自定义sql为主表的所有从表，对每个从表进行重新关联
                            if (itemObj.item == currTbNm) {//保存更新的sql
                                $scope.dataJson[idx]['sql'] = rs.sql;
                            }
                            if (itemObj && itemObj.relation && itemObj.relation.relationdetail && itemObj.relation.relationdetail.length > 0){
                                if (itemObj.relation.relationdetail[0].target == currTbNm){//找到那些以当前自定义sql为主表的从表
                                    //重新自动关联
                                    var relationObj = autoRelation(currTbNm, itemObj.item);
                                    $scope.dataJson[idx]['relation'] = relationObj['relation'];//更新保存关联结果
                                }
                            }
                        });
                    } else {
  ...
  /*
  *弹出当前表的字段选择窗口
  * 当点击确定时需要在datajson中记录那些是需要显示、sum、groupby等的字段，注意回显
  */
    $scope.openFieldSelectDialog = function(tableName,keyIndex){
        if (tableName && /^userdefinedsql\d/.test(tableName)){//如果是自定义sql就不执行
            return;
        }
        ///如果是第一张拖拽表，就不能弹出字段选择项，也不能有子查询
        if (tableName == $scope.draged[0].table) {
            if (keyIndex == 0) {//去掉第一个拖拽表的点击事件
                return;
            }
            $("body").mLoading();//遮罩层
            var relationObj = {'item':tableName};
            var addRelationObj = true;//将relationObj添加进$scope.dataJson（分修改还是直接添加的情况，因为选择字段的原因所以要做此调整）
            $scope.dataJson.map(function (item, idx) {//如果当前relationObj是编辑，则只进行修改
                if (item.item == tableName){//这里的编辑不能对第一个拖拽项进行编辑，否则会造成没有主关联表的情况
                    addRelationObj = false;
                }
            });
            if (addRelationObj) {//否则进行新增
                $scope.dataJson.push(relationObj);
            }
            $scope.genMapByDataJson();
            $scope.getGridColumn();
        ...
        var currTbCols = getTableColsFromPending(tableName,null,'showAll');
        if (currTbCols.length == 0){
            currTbCols = getTableColsFromAll(tableName);
        }
        var modalInstance = $uibModal.open({
            animation: true,
            ariaLabelledBy: 'modal-title',
            ariaDescribedBy: 'modal-body',
            templateUrl: '/dashboard/selectColumnsModel',
            controller: 'selectColumnsController',
            backdrop: false,
            resolve: {
                columns: function () {
                    return angular.copy(currTbCols);
                }
            }
        });
        modalInstance.result.then(function (rs) {
            if(rs.status && rs.status == 'success'){
                //svg画关联关系图
                $("body").mLoading();//遮罩层
                var currTableObj = $scope.draged[$scope.draged.length - 1];//当前拖入的tag
                for (var dgcnt = 0; dgcnt < $scope.draged.length - 1; dgcnt++) {//拖入重复表处理
                ...
                updateProposeColumns(rs.data,'y');//将重新配置的column添加到$scope.proposeColumns
                ...
                $timeout(function () {
                    ...
                    //从dataJson中查找当前表的主表
                    var mainTableName = '';
                    $scope.dataJson.map(function (item, idx) {
                        if (item.item == tableName){
                            if (item.relation && item.relation.relationdetail && item.relation.relationdetail[0].target && !mainTableName){//只取第一个作为其主表
                                mainTableName = item.relation.relationdetail[0].target;
                            }
                        }
                    });
                    if (!mainTableName){//空值判断,如果没找到就用第一个做主表
                        mainTableName = $scope.dataJson[0].item;
                    }
                    //生成关联关系，将关联关系保存到$scope.dataJson
                    relationObj = autoRelation(mainTableName, tableName);
                    relationObj['tbColumns'] = rs.data;
                    // console.log('-----autoRelation=',relationObj);

                    var addRelationObj = true;//将relationObj添加进$scope.dataJson（分修改还是直接添加的情况，因为选择字段的原因所以要做此调整）
                    $scope.dataJson.map(function (item, idx) {//如果当前relationObj是编辑，则只进行修改
                        if (item.item == tableName){//这里的编辑不能对第一个拖拽项进行编辑，否则会造成没有主关联表的情况
                            if (idx == 0) {//对第一个拖拽项只修改tbColumns不修改relation
                                // $scope.dataJson[idx]['tbColumns'] = rs.data;第一个拖拽表不需要弹出字段选择框
                            }else{
                                $scope.dataJson[idx] = relationObj;
                            }
                            addRelationObj = false;
                        }
                    });
    ...
    //更新proposeColumns,如果存在其中就更新，如果不存在就添加,status确定是更新还是覆盖
    function updateProposeColumns(newColumns,status){
        if(status && status == 'y'){//status==y表示覆盖。如果是覆盖，需要先根据传入的newColumns删除proposeColumns里的元素
            var deleteIdxFromProposeColumns = new Array();
            newColumns.map(function (item) {
                $scope.proposeColumns.map(function (itemProCol, idx) {
                    if (itemProCol.table == item.table && itemProCol.Field == item.Field) {
                        deleteIdxFromProposeColumns.push(idx);
                    }
                })
            });
            if(deleteIdxFromProposeColumns.length > 0){
                deleteIdxFromProposeColumns.map(function (item,idx) {
                    $scope.proposeColumns.splice(item - idx,1);//定点删除proposeColumns中的元素，别乱改删错了
                });
            }
        }
        if (newColumns && newColumns.length > 0) {
            var addNewColCount = 0;//如果newColumns在proposeColumns中不存在就新增
            if (status != 'y') {//由于有定点删除，所以当为y的时候不需要再查找，直接添加到$scope.proposeColumns即可
                $scope.proposeColumns.map(function (itemProCol,idx) {//为了将newColumns中的col更新到$scope.proposeColumns
                    if (addNewColCount < newColumns.length) {
                        newColumns.map(function (item) {
                            if (itemProCol.table == item.table && itemProCol.Field == item.Field) {
                                $scope.proposeColumns[idx] = item;
                                addNewColCount += 1;
                            }
                        })
                    }
            ...
            if (addNewColCount == 0) {
                newColumns.map(function (item) {
                    $scope.proposeColumns.push(item);
                })
            }
        } else {
            console.log('----newColumns is null-----');
        }
...
//jquery获取path和relation添加到组件之间
    ...
    $scope.dropCallback = function () {
        ...
        //弹出字段选择框,对字段进行选择，修改datajson中的值，对应view文件里取数据也需要改动
        $scope.openFieldSelectDialog($scope.draged[$scope.draged.length - 1].table);
        // //svg画关联关系图
        // $("body").mLoading();//遮罩层
        // $timeout(function () {
        ...
        //     console.log('---$scope.draged=',$scope.draged);
        //     console.log('---$scope.datagrid=',$scope.datagrid);
        //
        // });
        // $(".drag-content").removeClass('hide');
...
// },true);
    ...
    $scope.$on('ngRepeatFinished', function () {//dom加载完成后执行,用directive创建指令
        ...
        //如果是编辑状态，需要将以前保存的配置字段项赋值到$scope.proposeColumns进行回显
        $scope.dataJson.map(function (item) {
            if (item['tbColumns']) {
                updateProposeColumns(item['tbColumns']);//需要去重复
            }
        });
...
//从datajson中获取树结构
    function getTreeStructFromDataJson(dataJson) {
        ...
        while (relat2Relat.length > added) {
            ...
            if (!isadded || isadded != 'added') {//如果当前relat2Relat[i]没有存放到树形结构中则把当前object放到最后遍历，因遍历当前obj而其父节点还未生成(当修改父节点出现顺序打乱的情况)
                relat2Relat.push(relat2Relat[added]);
                let idx, djObj;
                tempDataJson.map(function (itemObj, ix) {
                    if (itemObj.item && itemObj.item == relat2Relat[added].currTb) {
                        idx = ix;
                        djObj = itemObj;
                    }
                })
                if (djObj && idx) {//把djObj放到链表最后，在while循环中继续遍历
                    tempDataJson.splice(idx, 1);
                    tempDataJson.push(djObj);
                }
            }
            added += 1;
            if (added > 10000) {//如果出现无限循环情况（父节点修改关联关系连接到其子节点的情况）
                alert('关联关系错误！请重新建立！');
    ...
    //将itemObj存放进treeStruct，条件是子tablename和父table的关联表相同
    function addObjToTreeStruct(treeStruct, itemObj) {//传引用，直接存放
        if (treeStruct.tbName && treeStruct.tbName == itemObj.parentTbName) {
    ...
    function autoRelation(mainTableName, currTableName) {
        let relationObj = {};
        ...
        mainTbCols = getTableColsFromPending(mainTableName);
        if (mainTbCols.length == 0) mainTbCols = getTableColsFromPending(mainTableName,null,'showAll');
        currTbCols = getTableColsFromPending(currTableName);
        if (currTbCols.length == 0) currTbCols = getTableColsFromPending(currTableName,null,'showAll');
        //获取关联字段
...
//两个字符串的相似程度，并返回相似度百分比
  ...
  */
    $scope.getGridColumn = function () {
        $http({
            ...
            if (response.code == '1') {
                ...
                if ($scope.refreshUpdateTb && $scope.refreshUpdateTb.status) {//必须只执行一次,refreshUpdateTb保存的是编辑条件下面板中所有自定义sql
                ...
                //每次编辑需要将第一张拖入表(主表)和自定义sql的字段覆盖更新到proposeColumns
                var selfDesignSqlFieldArr = new Array();
                rs.map(function (item) {
                    var tableNameFromField = (item.field && item.field.indexOf('__')) ? item.field.split('__')[0]:'';
                    //如果是自定义sql需要将这些字段更新到proposeColumns，如果是非自定义sql字段，在拖拽的时候已经更新，这里不需要再更新
                    //自定义sql的字段放到这里更新是因为当弹出输入框，用户写入sql后点击保存会执行这个方法，以下的更新是去重复更新
                    if ((tableNameFromField && tableNameFromField == $scope.draged[0].table) || (tableNameFromField && /userdefinedsql\d/.test(tableNameFromField))){
                        var tempDict = {
                            'Field': item.field.split('__')[1],
                            'name': item.title,
                            'table': item.field.split('__')[0],
                            'type': item.type,
                            'ifshow': '1',
                            'checked': '1'
                        };
                        selfDesignSqlFieldArr.push(tempDict);
                    }
                });
                if(selfDesignSqlFieldArr.length > 0){
                    updateProposeColumns(selfDesignSqlFieldArr,'y');//将这个list更新进proposeColumns
                }
    ...
    function getTableColsFromAll(tableName, listTb) {
        ...
        if (listTb && listTb.trim() != '' && listTb == 'y') {
            tbColumns = $scope.columns.filter(function (index) {
                if (index.ifshow == '0') return false;
                let sel = false;
    ...
    //获取该表字段里可用的字段，选择那些用户筛选过的字段,
    //tableName,listTb（是否是素组的tableName）,showAll(所有字段还是筛选后的字段)
    function getTableColsFromPending(tableName, listTb, showAll) {
        if (!tableName || tableName.length == 0) {
            return [];
        }
        var tbColumns = [];
        if (listTb && listTb.trim() != '' && listTb == 'y') {
            tableName.map(function (tbName) {
                var currTbCols = [];
                $scope.proposeColumns.map(function (colObj) {
                    if (!showAll && (colObj.ifshow == '0' || colObj.checked == '0')) {
                        //当这种条件的时候就pass，但不能不要这个判断（做反向条件太麻烦）
                    } else if (tbName == colObj.table) {
                        currTbCols.push(colObj);
                    }
                })
                if (currTbCols.length == 0){//如果当前表是用户选择为空的(选择有字段改为全部选空)，那么也需要把它的所有字段加到tbColumns，在关联条件里使用
                    $scope.proposeColumns.map(function (colObj) {
                        if (tbName == colObj.table) {
                            currTbCols.push(colObj);
                        }
                    })
                }
                tbColumns = tbColumns.concat(currTbCols);
            });
        } else {
            tbColumns = $scope.proposeColumns.filter(function (index) {
                if (!showAll && (index.ifshow == '0' || index.checked == '0')) return false;
                return index.table == tableName;
            });
        }
        return tbColumns;
  ...
  */
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                ...
                allTbCols: function () {
                    ...
                    var temp = getTableColsFromPending(dragedTbs, 'y');//拖入面板的所有表的字段
    ...
    //检查中文title是否存在特殊字符串，如果存在需要用户更改
    function checkChineseTitleIllegal(){
        var replaceChar = ['.','$','@','&','+','-','*','/','(',')'];
        var illegalTitleCount = 0;
        $scope.datagrid.columns.map(function (colObj) {
            if (colObj.title && isContainIllegalChar(replaceChar,colObj.title)['status']) {
                illegalTitleCount += 1;
            }
        ...
        if (illegalTitleCount > 0) {
            ...
            if (confirm('有非法的中文标头' + illegalTitleCount + '个！（中文表头含有 .、$、@、&等符号）\n 是否需要用双下划线替换掉这些符号？')) {
                $scope.datagrid.columns.map(function (colObj, index) {
                    $scope.datagrid.columns[index].title = isContainIllegalChar(replaceChar,colObj.title)['rsData'];
                })
            } else {
                return false;
            }
    ...
    function isContainIllegalChar(aimChar,itemchar){
        var rsObject = {'status':false,'rsData':itemchar};
        var breakOutNum = 0;
        aimChar.map(function (aimchar) {
            if (itemchar){
                while (itemchar.indexOf(aimchar) != -1 && aimchar) {//经过多次替换，直到把所有特殊字符都替换完
                    rsObject['rsData'] = itemchar.replace(aimchar, '__');
                    rsObject['status'] = true;
                    itemchar = rsObject['rsData'];
                }
            }
        })
        return rsObject;
  ...
  */
    $scope.save = function () {
        ...
        var checkChineseTitleStatus = checkChineseTitleIllegal();
        if (!checkChineseTitleStatus){//如果校验没有被处理（用户不需要程序自动修改特殊字符）就返回
            return false;
        }
        //中文title判断结束
...
//当用户拖入表控件时，提示用户选择那些字段需要、需要sum还是groupby
//ifshow控制表格的字段显示，checked控制弹出框选择字段是否勾选，不能只是用ifshow，因为当弹出框不选任何字段那么查询出来的columnName就不能显示表格数据
app.controller('selectColumnsController', function ($scope, $http, $uibModalInstance, columns) {
    let columnSelectedCount = 0;
    columns.map(function (val, index) {
        if (val.ifshow == "1") {
            if(!columns[index].checked){
                columns[index].checked = '0';//讨论结果为默认不勾选字段，让用户勾选，如需要默认勾选删除本行放开以下注释即可
            }
            // columns[index].checked = '1';
            // columnSelectedCount += 1;
            // if (columns[index] && !columns[index].operator) {//如果没有operator就执行自动选择，如果是编辑就回显编辑的结果
            //     columns[index].operator = getOperateByColType(columns[index].type, columns[index].Field);
            // }
        } else {
            columns[index].checked = '0';
        }
    });
    //根据类型和字段名自动判断是sum还是groupby(用尽一切办法尽量考虑返回正确的匹配)，只有90%的准确率，需要用户检查
    function getOperateByColType(typeStr,field){
        var typeStr = typeStr.toLowerCase();
        var field = field ? field.toLowerCase() : '';

        if (typeStr && typeStr.trim()){
            if (typeStr.indexOf('int') > -1 || typeStr.indexOf('float') > -1 || typeStr.indexOf('double') > -1 ||
                typeStr.indexOf('number') > -1 || typeStr.indexOf('decimal') > -1 || typeStr.indexOf('integer') > -1||
                typeStr.indexOf('real') > -1) {
                if (field && (field.indexOf('id') > -1 || field.indexOf('depart') > -1 ||
                    field.indexOf('create') > -1 || field.indexOf('update') > -1)) {
                    return 'group by';
                }else{
                    return 'sum';
                }
    ...
    $scope.doSelectAll = function () {
        if ($scope.selectAll) {
            columns.map(function (val, index) {
                $scope.columns[index].checked = '1';
                $scope.columns[index].ifshow = 1;
                if ($scope.columns[index] && !$scope.columns[index].operator) {//如果没有operator就执行自动选择，如果是编辑就回显编辑的结果
                    $scope.columns[index].operator = getOperateByColType($scope.columns[index].type,$scope.columns[index].Field);
                }
            })
        } else {
            columns.map(function (val, index) {
                $scope.columns[index].checked = '0';
                $scope.columns[index].ifshow = 0;
            })
        }
    }
    ...
    $scope.fn = function (a, b, checkBoxClick) {
        if (!checkBoxClick) {//if clicked the label
            if ($scope.columns[a].checked == '1') {
                $scope.columns[a].checked = '0';
                $scope.columns[a].operator = null;
            }else{
                $scope.columns[a].checked = '1'
                //选择的时候自动添加操作类型
                if ($scope.columns[a] && !$scope.columns[a].operator) {//如果没有operator就执行自动选择，如果是编辑就回显编辑的结果
                    $scope.columns[a].operator = getOperateByColType($scope.columns[a].type, $scope.columns[a].Field);
                }
            }
            b = angular.copy($scope.columns[a].checked);
        }
    ...
    $scope.ok = function () {
        columns = angular.copy($scope.columns);
        if(checkSelectedColOperator(columns)){//确认之前需要确定该填的都填写了
            $uibModalInstance.close({'status':'success','data':columns});
        }
    };
    //检查如果勾选了字段但没有选择operator需要给出提示并禁止提交，否则后台会拼接sql出错，不过这里由于自动选择功能使得不可能存在选空的情况
    function checkSelectedColOperator(columns) {
        var nullOperator = 0;//有多少个operator没有选
        columns.map(function (item) {
            if (item.checked == '1' && !item.operator){
                nullOperator += 1;
            }
        });
        if (nullOperator > 0) {
            alert('您有 ' + nullOperator + ' 个字段的计算方式没有选择！');
            return false;
        }
        return true;
    }
