        ...
        if (!options || options.add !== true/* || loaded_node_id === null*/) {
            ...
            for (i = 0, len = r.length; i < len; i++) {
                ...
                if (updated[record.id] == true) {
                    ...
                    //js异步绘制树 jnp 2010-10-8
                    /*var runner = new Ext.util.TaskRunner();
                    runner.start({
                    run: function() {
                    arguments[0].fireEvent("add", arguments[0], arguments[1], arguments[2]);
                    return false;
                    },
                    args: [this, [record], this.data.indexOf(record)],
                    interval: 2, 
                    repeat: 1
                    });
                    temp += 7;
                    var delayedTask = new Ext.util.DelayedTask();
                    delayedTask.delay(temp, function() {
                    arguments[0].fireEvent("add", arguments[0], arguments[1], arguments[2]);
                    }, this, [this, [record], this.data.indexOf(record)]);*/
                    //js异步绘制树END
                    if (TEMPNEED == true) {
                        TEMPARR.push([this, [record], this.data.indexOf(record)]);
                        if (i == len - 1) {
                            var firstNode = TEMPARR.shift();
                            this.fireEvent("add", this, firstNode[1], firstNode[2]);
                        }
                    }
                    else {
...
Ext.ux.maximgb.tg.AdjacencyListStore = Ext.extend(Ext.ux.maximgb.tg.AbstractTreeStore,
{
    ...
    getRootNodes : function()
    {
        ...
        for (i = 0, len = records.length; i < len; i++) {
            ...
            var pid = records[i].get(this.parent_id_field_name);    // edit by ray - 2010.04.20
            if (pid !== 0 && !pid) {
