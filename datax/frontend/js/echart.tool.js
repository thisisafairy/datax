...
function buildEchart(config, element, sourcedata, size, scenetype, itemConfig, itemConfigs, linkStatus) {
    ...
    var data;
    var mainCanvas = element[0], width, height, chart;
    scope.config = config || {};
    ...
    if (typeof sourcedata == 'object') {
        ...
        var dataSource = dl.load({ url: encodeURI(config.data.url) });
    ...
    function getSizes(config) {
        var col_parent = $(mainCanvas).closest(".cell-box");
        var parent = col_parent.length == 0 ? $(mainCanvas).closest(".box") : col_parent;
        height = config.height || 240;
        mainCanvas.style.height = height + 'px';
        mainCanvas.style.width = (parent.length > 0 ? parent.width() : config.width || 320) + 'px';
    }
    ...
    function setOption() {
        ...
        var chartData = util.filterData(scope.data, scope.config);
        if (scope.config.data.type === 'single') {
            ...
            // region 联动配置
            if (scenetype && scenetype == 'dashbboard2') {
                if (itemConfig.data.events && (Object.keys(itemConfig.data.events)).length > 0) {
                    if (itemConfig.data.events.type == 'redirect') {
                        chart.on('click', function (param) {
                            // console.log('itemConfig', itemConfig)
                            var tempUrl = itemConfig.data.events.linkUrl;
                            if (itemConfig.data.events.isComponentArg) {
                                tempUrl = addArgToUrl(tempUrl, itemConfig.data.events.componentUrlArg + '(_)equal', util.eventDict(scope.config.data.type, param))
                            }
                            if (itemConfig.data.events.isCustomArg && itemConfig.data.events.customUrlArg) {
                                if (tempUrl.split('?').length == 1) {
                                    tempUrl = tempUrl + '?' + itemConfig.data.events.customUrlArg;
                                } else {
                                    tempUrl = tempUrl + '&' + itemConfig.data.events.customUrlArg;
                                }
                            }
                    ...
                    if (itemConfig.data.events.type == 'relation') {
                        chart.on('click', function (param) {
                            itemConfigs.forEach(function (__val) {
                                var __type = __val.data.dropType ? __val.data.dropType : __val.data.datatype;
                                var _refresh = false;
                                for (var i = 0; i < itemConfig.data.events.relationInfo.length; i++) {
                                    if ('chart' == __type) {
                                        if (__val.data.id == itemConfig.data.events.relationInfo[i].chart) {
                                            var _chartObj = JSON.parse(__val.data.echartconfig);
                                            _chartObj.data.url = addArgToUrl(_chartObj.data.url,
                                                itemConfig.data.events.relationInfo[i].olapCol + '(_)equal',
                                                util.eventDict(scope.config.data.type, param));
                                            __val.data.echartconfig = JSON.stringify(_chartObj);
                                            _refresh = true;
                                            break;
                                        }
                                    } else if ('table' == __type) {
                                        if (__val.data.id == itemConfig.data.events.relationInfo[i].chart) {
                                            __val.data.jsonconfig.data.url = addArgToUrl(__val.data.jsonconfig.data.url,
                                                itemConfig.data.events.relationInfo[i].olapCol + '(_)equal',
                                                util.eventDict(scope.config.data.type, param));
                                            _refresh = true;
                                            break;
                                ...
                                if (_refresh) {
                                    itemConfig.data['link'] = 'linkStart';
                                    __val.data.refreshCount += 1;
                                }
                            });
                        });
                    }
                ...
                if (scope.config.event && scope.config.event.eventtype == 'relation' && scope.config.event.source) {
                    chart.on('click', function (param) {
                        doRelation(param, scope.config.event);
                    });
                }

                if (scope.config.event && scope.config.event.eventtype == 'redirect' ){
                    chart.on('click', function (param) {
                        window.open(scope.config.event.url + '?' + scope.config.event.column + '=' + util.eventDict(scope.config.data.type, param));
                    });
                }
            }
            // endregion
            var option = getOptions(chartData, scope.config, scope.config.data.type,mainCanvas);

            // 2018年8月29日15:18:43 增加代码用于配置x、y轴颜色
            if (config.axisLabel) {
                option['axisLabel'] = config.axisLabel;
            }
...
                            }
                ...
                if(config.field.start&&config.field.start.field)//K线图配置项
                {
                    ...
                    data.map(function (a) {
                        try{
                            ...
                            console.log('----error-----echarts.tool.js;line:977;method:data.map(funtion(a){})-this can be happned-',err.message);
                        }
                    });
                   rs.push({
                        type: 'candlestick',
    ...
    function getVisualMap(data, config) {
        ...
        function getGraphic(rsdata, config, panel) {
            ...
            config.field.y.map(function(row){
                ...
                if(config.field.x&&config.field.y[0].aggregate=="rank")//单指标排名
                {
                    ...
                    // var tcolumn = config.field.group;
                    // var url = config.data.url.indexOf("?")>0?config.data.url:(config.data.url+"?");
                    // url += "gcol="+gcolumn.field+"&kpicol="+row.field;
                    // if(tcolumn)
                    //     url+="&tcol="+tcolumn.field;
                    // var filters = SinglefilterJSON(config);
                    // if(filters.length>0)
                    //     url += "&filters="+JSON.stringify(filters);
                    //
                    // var dataSource = dl.load({ url: encodeURI(url) });
                    // data = JSON.parse(dataSource.replace(/NaN/g,'\"\"').replace(/Null/g,'\"\"'));
                    var gcolumn = config.field.x;
                    var url = '';
        ...
        function buildListCss(index, row, summry) {
                canvaspanel.find('.card-' + index).css({ 'color': row._titleColor, 'font-size': row._titleFontsize + 'px','position':'relative' }).html('<div class="lst_title" style="float:left">'+row.label +': </div>'+ '<div class="lst_val"></div>');
                if(row.plusicon&&row.plusicon==="true")
                {
                    var icon = "right";var color="";
                    if(summry>0)
                    ...
                    if(row.aimfloat=="right")
                        canvaspanel.find('.card-' + index).find('div.lst_val').css({'position':"absolute",'right':0});
                }
                else
                {
