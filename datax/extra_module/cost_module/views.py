...
# 组件非硅成本公式
def comnonguiCost(glassPrice, now_rate, materialestimated, component_sum_bom, componentinstockgood, avgrate, batteryare,
                  tabletsize, CTM, wrate):
    ...
    return (glassPrice / (1 + now_rate) + materialestimated + component_sum_bom) / componentinstockgood / (
            avgrate * batteryare * tabletsize * CTM * (1 - wrate))
...
def combatCost(singlew_cost, batteryare, avgrate, tabletsize, piesrate, componentinstockgood, CTM, wrate):
    try:
        ...
        a = (singlew_cost * batteryare * avgrate * tabletsize * (1 + piesrate)) / componentinstockgood / (
                avgrate * batteryare * tabletsize * CTM * (1 - wrate))
    except Exception as e:
...
def saveTaxrate(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            executeSession.executeSql(type, table, id, taxrate=taxrate, createid=createid, createname=createname,
    ...
    try:
        for data in datas:
            if 'id' in data:
                ...
                executeSession.executeSql(type, table, id=data.get('id'), productsize=data['productsize'],
                                          tabletsize=data['tabletsize'],
                                          guicost=data['guicost'], convertrate=data['convertrate'],
                                          singlew=data['singlew'], inbad=data['inbad'],
                                          instockgood=data['instockgood'], avgrate=data['avgrate'], ctm=data['ctm'],
                                          wrate=data['wrate'],
                                          piesrate=data['piesrate'], zinstockgood=data['zinstockgood'],
                                          aimw=data['aimw'], modifyid=userid,
                ...
                executeSession.executeSql(type, table, id=id, productsize=data['productsize'],
                                          tabletsize=data['tabletsize'], guicost=data['guicost'],
                                          convertrate=data['convertrate'],
                                          singlew=data['singlew'], inbad=data['inbad'], instockgood=data['instockgood'],
                                          avgrate=data['avgrate'], ctm=data['ctm'], wrate=data['wrate'],
                                          piesrate=data['piesrate'],
                                          zinstockgood=data['zinstockgood'], aimw=data['aimw'], createid=userid,
                                          createname=username,
    ...
    try:
        for id in ids.split(','):
            if id:
                ...
                delsql = '''DELETE FROM cost_trend WHERE productsize = '{}' and tabletsize = '{}' '''.format(trend[0],
                                                                                                             trend[1])
    ...
    try:
        for data in datas:
            if 'id' in data:
                ...
                executeSession.executeSql(type, table, id=data.get('id'), costtype=data['costtype'],
                                          productsize=data['productsize'],
                                          capacityuse=data['capacityuse'], batterywage=data['batterywage'],
                                          modulewage=data['modulewage'], modifyid=userid,
                                          modifyname=username, modifytime=current_time)
            else:
                id = UUIDTools.uuid1_hex()
                type = 'insert'
                table = '"public"."cost_person"'
                executeSession.executeSql(type, table, id=id, costtype=data['costtype'],
                                          productsize=data['productsize'],
                                          capacityuse=data['capacityuse'], batterywage=data['batterywage'],
...
def batteryBOMList(request):
    ...
    if productsize:
        ...
        try:
            ...
            allList = executeSession.getArrResult(sql, autoClose=True)
            # sum(预估金额)
            sumestimatedamountsql = r'''SELECT sum(estimatedamount) FROM "public"."cost_bom" where productsize='{}' '''.format(
                productsize)
        ...
        try:
            ...
            for data in datas:
                ...
                if 'id' in data:
                    type = 'update'
                    id = data.get("id")
                    executeSession.executeSql(type, table, id, productsize=productsize, datatype=datatype, name=name,
                                              materialtype=materialtype, code=code, spec=spec,
                                              unit=unit, bom=bom, badcost=badcost, badbom=badbom, rateprice=rateprice,
                                              percent=percent, estimatedamount=estimatedamount,
                                              modifytime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                              modifyid=user_id, modifyname=user_name)
                else:
                    type = 'insert'
                    id = UUIDTools.uuid1_hex()
                    executeSession.executeSql(type, table, id, productsize=productsize, datatype=datatype, name=name,
                                              ...
                                              percent=percent, estimatedamount=estimatedamount,
                                              createtime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                              createid=user_id, createname=user_name)
            resultObj['code'] = '1'
        except Exception as e:
            resultObj = tools.errorMes(e.args)
            resultObj['code'] = '0'
        ...
        for sheetName in sheetNames:
            ...
            for i in range(1, rowCount):
                ...
                if args:
                    ...
                    executeSession.executeSql(type, table, id, productsize=productsize, tablesize=int(args[0]),
                                              datatype=rows[2], materialtype=rows[3], code=rows[5], name=rows[4],
                                              spec=rows[6], unit=rows[7], bom=rows[8], badcost=rows[9], badbom=rows[10],
                                              rateprice=rows[11], percent=rows[12],
                                              estimatedamount=estimatedamountmoney, createtime=current_time)
                # 电池BOM
                else:
                    ...
                    executeSession.executeSql(type, table, id, productsize=productsize,
                                              ...
                                              rateprice=rows[10], percent=rows[11],
                                              estimatedamount=estimatedamountmoney,
...
def componentBOMList(request):
    ...
    if productsize and module:
        executeSession = sqlutils.SqlUtils(dbType="extension")
        try:
            sql = r'''SELECT * FROM "public"."cost_bomz" where productsize = '{}' and tablesize = '{}' '''.format(
                productsize, module)
            allList = executeSession.getArrResult(sql, autoClose=True)
            # sum(预估金额)
            sumestimatedamountsql = r'''SELECT sum(estimatedamount) FROM "public"."cost_bomz" where productsize = '{}' and tablesize = '{}' '''.format(
                productsize, module)
            sumestimatedamount = executeSession.getArrResult(sumestimatedamountsql, autoClose=True)[0][0]
            resultObj['sumestimatedamount'] = sumestimatedamount
            resultObj['rows'] = allList
        ...
        try:
            ...
            for data in datas:
                ...
                if 'id' in data:
                    type = 'update'
                    id = data.get("id")
                    executeSession.executeSql(type, table, id, productsize=productsize, tablesize=tablesize,
                                              datatype=datatype, name=name, materialtype=materialtype, code=code,
                                              spec=spec,
                                              unit=unit, bom=bom, badcost=badcost, badbom=badbom, rateprice=rateprice,
                                              percent=percent, estimatedamount=estimatedamount,
                                              modifytime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        ...
        try:
            ...
            for data in datas:
                ...
                if 'id' in data:
                    type = 'update'
                    id = data.get("id")
                    executeSession.executeSql(type, table, id, price=price, productsize=productsize,
                                              modifytime=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                              modifyid=user_id, modifyname=user_name)
                else:
                    type = 'insert'
    ...
    if request.method == "POST":
        ...
        def fetchall(aim):
            ...
            try:
                ...
                # 组件非硅成本 版本保存
                if isshuangbo:
                    component_nongui_cost = newComnonguiCost(component_sum_bom, componentinstockgood, avgrate,
                                                             batteryare, tabletsize, CTM, wrate)
                else:
                    component_nongui_cost = comnonguiCost(bolicost, now_rate, materialestimated, component_sum_bom,
                                                          componentinstockgood, avgrate, batteryare, tabletsize, CTM,
                                                          wrate)
                # 所有方案都受到人工制费的影响
                for i in personcost:
                    if i[0] == productsize:
                        ...
                        # 电池成本
                        batterycost = combatCost(singlew_cost, batteryare, avgrate, tabletsize, piesrate,
                                                 componentinstockgood, CTM, wrate)
                        ...
                        id = UUIDTools.uuid1_hex()
                        costtrend = (
                            id, batterycost, component, batteryaftcost, componentaftrate, guicost, productsize,
                            tabletsize,
                            capacityuse, createid, createname, createtime, spec, gui, battery_nongui_cost,
                            component_nongui_cost, singlew_cost)
        ...
        try:
            for fileds in map(fetchall, aimall):
                for filed in fileds:
                    ...
                    executeSession.executeSql(type, table, filed[0], battery=filed[1], component=filed[2],
                                              batteryaftrate=filed[3], componentaftrate=filed[4], guicost=filed[5],
                                              productsize=filed[6], tabletsize=filed[7], capacityuse=filed[8],
                                              createid=filed[9],
                                              createname=filed[10], createtime=filed[11], datatype=filed[12],
                                              batterygui=filed[13], batterynongui=filed[14], componentnongui=filed[15],
                                              batterysinglew=filed[16])
        except Exception as e:
            print('/cost/saveCostVersion ---', e)
        ...
        try:
            ...
            if not executeSession.getArrResult(versionsql, autoClose=True):
                ...
                id = UUIDTools.uuid1_hex()
                executeSession.executeSql(type, table, id, versionsaverid=createid, versionsaver=createname,
                                          versionsavetime=createtime)
            resultObj['code'] = '1'
        except Exception as e:
            resultObj = tools.errorMes(e.args)
            resultObj['code'] = '0'
...
def regenerateData(request):
    ...
    perbatterycostsql = r'''SELECT batterywage FROM cost_person WHERE productsize='{}' AND capacityuse='{}' '''.format(
        productsize, batterycapacityuse)
    prebatterywage = executeSession.getArrResult(perbatterycostsql, autoClose=True)[0][0]

    permodulewagecostsql = r'''SELECT modulewage FROM cost_person WHERE productsize='{}' AND capacityuse='{}' '''.format(
        productsize, componentcapacityuse)
    permodulewage = executeSession.getArrResult(permodulewagecostsql, autoClose=True)[0][0]

    try:

        if first:
            glasssql = r''' select distinct  spec,rateprice from cost_bomz where productsize='{}' and datatype='玻璃' LIMIT 1'''.format(
                productsize)
            ...
            # 获取人工制费
            batterycapacityusesql = r''' SELECT batterywage FROM cost_person WHERE productsize='{}' AND capacityuse='{}' '''.format(
                productsize, batterycapacityuse)
            prebatterywage = executeSession.getArrResult(batterycapacityusesql, autoClose=True)[0][0]
            componentcapacityusesql = r'''SELECT modulewage FROM cost_person WHERE productsize='{}' AND capacityuse='{}' '''.format(
                productsize, componentcapacityuse)
            permodulewage = executeSession.getArrResult(componentcapacityusesql, autoClose=True)[0][0]
            # 获取材料组合
            # isshuangbo 双波 ture
            if not isshuangbo:
                materialssql = r'''SELECT   distinct code,spec  FROM "public"."cost_bomz"  WHERE datatype='材料组合' AND productsize='{}' LIMIT 1'''.format(
                    productsize)
                ...
                for component in component_bom:
                    if component[0] == productsize == aimproductsize and component[1] == tabletsize:
                        if isshuangbo:
                            ...
                            # 获取到材料对应的编码
                            codesql = r'''SELECT   distinct code,spec  FROM "public"."cost_bomz"  WHERE datatype='材料组合' AND productsize='{}' AND spec = '{}' '''.format(
                                productsize, spec)
                            code = executeSession.getArrResult(codesql, autoClose=True)[0][0]
                            # 获取到材料方案价格
                            materialsql = r'''SELECT  estimatedamount  FROM "public"."cost_bomz" WHERE code = '{}' AND productsize='{}' AND tablesize='{}' LIMIT 1'''.format(
                                code, productsize, tabletsize)
                            materialestimated = executeSession.getArrResult(materialsql, autoClose=True)[0][0]
                        component_sum_bom = component[2]
                        # 电池singlew_cost成本   每瓦成本：硅片+非硅+人工制费
                        battery_nongui_cost = batnonguiCost(battery_sum_bom, singlew, batteryinstockgood, inbad,
                                                            now_rate)
                        # 电池硅成本
                        ...
                        # 电池成本
                        battery_const = combatCost(singlew_cost, batteryare, avgrate, tabletsize, piesrate,
                                                   componentinstockgood, aimCTM, wrate)
                        if tabletsize == 60:
                            try:
                                # 是否有玻璃的价格没有就为0不参与计算
                                isglasspricesql = r''' select distinct  spec,rateprice from cost_bomz where datatype='玻璃' and productsize='{}' and tablesize='{}' '''.format(
                                    productsize, tabletsize)
                            ...
                            # 组件非硅成本 双玻为新公式
                            if isshuangbo:
                                component_nongui_cost = newComnonguiCost(component_sum_bom, componentinstockgood,
                                                                         avgrate, batteryare, tabletsize, aimCTM, wrate)
                            else:
                                # todo
                                component_nongui_cost = comnonguiCost(totalGlassPrice, now_rate, materialestimated,
                                                                      component_sum_bom, componentinstockgood, avgrate,
                                                                      batteryare, tabletsize, aimCTM, wrate)
                            print(component_nongui_cost)
                            print(component_nongui_cost)
                            print(component_nongui_cost)
                            print(component_nongui_cost)
                            print(component_nongui_cost)
            ...
            try:
                for component in component_bom:
                    if component[0] == productsize == aimproductsize and component[1] == tabletsize:
                        if not isshuangbo:
                            # 获取到材料对应的编码
                            codesql = r'''SELECT  code  FROM "public"."cost_bomz"  WHERE datatype='材料组合' AND productsize='{}' AND spec = '{}' '''.format(
                                productsize, spec)
                            code = executeSession.getArrResult(codesql, autoClose=True)[0][0]
                            # 获取到材料方案价格
                            materialsql = r'''SELECT  estimatedamount  FROM "public"."cost_bomz" WHERE code = '{}' AND productsize='{}' AND tablesize='{}' LIMIT 1'''.format(
                        ...
                        if tabletsize == 60:
                            ...
                            if isshuangbo:
                                component_nongui_cost = newComnonguiCost(component_sum_bom, componentinstockgood,
                                                                         avgrate, batteryare, tabletsize, CTM, wrate)
                            else:
                                component_nongui_cost = comnonguiCost(totalGlassPrice, now_rate, materialestimated,
                                                                      component_sum_bom, componentinstockgood, avgrate,
                                                                      batteryare, tabletsize, CTM, wrate)
                            ...
                            # 组件每瓦成本(未税)
                            component_singlew_cost = comsinglewCost(component_nongui_cost, procurement_cost,
                                                                    permodulewage)
                            resultnongui['component60'] = '{:0.4f}'.format(component_nongui_cost)
                            resultsinglewnonrax['component60'] = '{:0.4f}'.format(component_singlew_cost)
...
def getLinkageData(request):
    ...
    if productsize:
        executeSession = sqlutils.SqlUtils(DBTypeCode.EXTENSION_DB.value)
        sql = r'''SELECT distinct spec  FROM cost_bomz WHERE datatype='材料组合' AND productsize='{}' '''.format(
            productsize)
        objs = executeSession.getArrResult(sql, autoClose=True)
        results = {}
        result = []
        ...
        # 查找外购电池成本
        buybatterypricesql = r'''SELECT DISTINCT  price from cost_purchase where productsize = '{}' '''.format(
            productsize)
        try:
...
def getdinguserinfo(request):
    ...
    # 获取 access_token
    data = urllib.parse.urlencode({'appkey': 'dingee7y1sxlfqzbd2ga',
                                   'appsecret': 'AtSPhx9JH6HNZb_dkBoi4DOo88KDVYthHscZ-7D5iT5Y5bWV5b3b4B_HY4yOQo-a'})
    ...
    response = urllib.request.urlopen(
        'https://oapi.dingtalk.com/user/get?access_token=' + access_token + '&userid=093111674520342683')
    ...
    for item in items:
        ...
        if item['data']['id'] == '826D7F55E5498B2DCCBC7BA540E97621':
            if parentDepartmentname == '集团':
                executeSession = sqlutils.SqlUtils(DBTypeCode.EXTENSION_DB.value)
                sql = r'''select distinct departmentname from oa_department_attendance where superdep='{}' '''.format(
                    departmentname)
    ...
    executeSession = sqlutils.SqlUtils(DBTypeCode.EXTENSION_DB.value)
    insertsql = r'''insert into oa_cache_ding(department,superdepartment,departmentid,parentid) values('{}','{}','{}','{}')'''.format(
        departmentname, parentDepartmentname, departmentid, parentid)
